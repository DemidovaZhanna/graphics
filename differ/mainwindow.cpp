...
std::pair<double, double> position_in_space (double m, double k, double g, double t,
											 double V_x0, double V_y0)
{
...
QScatterSeries* MainWindow::create_ser_without_resistance( double V_x, double V_y, double g, double x0, double y0)
{
	...
	for (double t = 0; t < 20; t += 0.01)
		series->append(x0 + V_x*t,y0 + V_y*t-g*t*t/2);

	return series;
}


QLineSeries* MainWindow::create_exact_solution ( double m, double V_x, double V_y, double k, double g, double x0, double y0)
{
	QLineSeries* series = new QLineSeries();
	series->setName("с учетом сопротивления(точное решение)");

	for (double t = 0; t < 20; t += 0.01)
		series->append(x0 + m*V_x/k*(1-exp(-k*t/m)),y0 + m/k*((V_y+m*g/k)*(1-exp(-k*t/m)) - g*t));

	return series;
}


QLineSeries* MainWindow::create_numer_approx ( const int N, double t, double k, double m, double g,
											   double V_x, double V_y, double x0, double y0)
{
	QLineSeries* series = new QLineSeries();
	...
	int i = 1;
	double *pY = new double [N]; pY[0] = y0;
	double *pX = new double [N]; pX[0] = x0;
...
QLineSeries* MainWindow::create_relativism ( double _h, double V_y, double V_x,
											 double m, double k, double g, int N, double x0, double y0)
{
	QLineSeries* series = new QLineSeries();
...
QLineSeries* MainWindow::create_arbitrary_angle (double m_p, double k, double V_xp,
												 ...
												 double t_detach, double x_det, double y_det, double x0, double y0)
{
	QLineSeries* series = new QLineSeries;
	series->setName("случай резкого изменения массы под произвольным углом:");
	...
	bool first = true;
	for (double t = 0; t < 20; t += 0.01) {
		double x_res = x_det + m_p*V_xp/k*(1 - exp(-k*(t-dt)/m_p));
		double y_res = y_det + m_p/k*((V_yp+m_p*g/k)*(1 - exp(-k*(t-dt)/m_p)) - g*(t-dt));
		if (first == true)
			series->append(x0 + x_res, y0 + y_res);
		else
			series->append(x_res, y_res);

		if(t >= t_detach && !detached){
			double V_x0 = V_xp*exp(-k/m_p * t);
			double V_y0 = V_yp*exp(-k/m_p * t) - g*m_p/k*(1 - exp(-k/m_p* t));
			x_det = x0 + m_p*V_xp/k*(1-exp(-k*t/m_p));
			y_det = y0 + m_p/k*((V_yp+m_p*g/k)*(1 - exp(-k*t/m_p)) - g*t);
	...
	, ui(new Ui::MainWindow)
{
	...
	QScatterSeries* series1 = create_ser_without_resistance( V_x, V_y, g, x0, y0);
	QLineSeries* series2 = create_exact_solution ( m, V_x, V_y, k, g, x0, y0);

	const int N = 20000;
	double dt = 0.01;

	QLineSeries* series3 = create_numer_approx ( N, dt, k, m, g, V_x, V_y, x0, y0);

	double V_x1 = 6,
		   V_y1 = 10;


	QLineSeries* series4 = create_relativism ( dt, V_y1, V_x1, m, k, g, N, x0, y0);
...
//	}
	...
	QLineSeries* series6 = create_arbitrary_angle( m_p, k, V_xp, V_yp, m_p2, g, V_xt, V_yt, t_detach, 0, 0, x0, y0);
